...
## Phase 3 — To the journey model ✅ SIMS COMPLETE (2026-07-09; T3.4 figures in flight)

**Outcomes:** T3.1 Sim 4 accepted 14/14 (descent ordering emergent from an audited no-ordering-term EFE in 24/24 seeds; forced access thickens 100%; rupture asymmetry 6.8x; habit/protector discriminant; mandate update rate 0; ablation degrades ordering to indifference). T3.2 Sim 5 accepted 11/1 (regulated vs fluent-but-threatened 18.1 vs 0; S5.2 ablation confirms regulation-channel mechanism; borrowed-then-owned ~7 sessions; A5.2b falsified-with-better-theory: regulation-only reduces capture 0.33 but revises 0 — regime and evidence separately necessary). **T3.3 Sim 7 accepted 24/24: THE JOURNEY MODEL EXISTS** — full biography in every seed, blind taxonomy recovery, melt inverts formation order, both control lives fail in their distinct predicted ways, component signatures reappear without retuning (composition verified as imports of audited modules). §9 extended accordingly; stub narrowed to formation-under-inferred-depth (the 6b clamp test). Note: gpt-5.6 unavailable on this Codex account (server-side 400 on CLI 0.144.0); default model used throughout.

*(Original directive: "all simulations, push till full journey model or we find it doesn't work" — it worked.)*
