...
What the model does not yet formalize is equally important. Protectors do not merely block. They negotiate. They assess trust. They have conditions under which they will step back. They often transform after the exile’s burden changes. Those dynamics are clinically central and formally deferred here.
...
Part A experiences withdrawal as abandonment and deadness. Part B experiences approach as exposure and danger. Each therefore escalates in response to the other.

Under low Self-energy, this produces the familiar phenomenology: ambivalence, reversals, exhaustion, and the feeling that each side is wholly true when it has the floor. That last feature matters. Polarization is not simply conflict between two simultaneously represented preferences. It is often alternation between rival local realities.
...
4. **Expository restructuring:** open with an IFS-native account of parts, Self, blending, witnessing, and unburdening in clinical language, then introduce the computational translation after the phenomenology and therapeutic logic are clear.  
5. **Visual program redesign:** benchmark the figure set against the CBT and coherence-therapy papers, identify the minimum visual grammar needed to match that standard, and intersperse explanatory figures throughout the paper rather than concentrating them at the end. A matrix-style visualization of the model is the minimum starting point.  
6. **Multi-part networks:** move from one polarity pair to several coupled bundles with different developmental ages and policies.  
7. **Empirical fitting:** align simulated trajectory measures with session-level data, especially revision order and generalization gradients.
