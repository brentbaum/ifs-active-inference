...
likely blended. The method proceeds by respecting that fact rather than
overriding it. Protectors are not treated as resistance. They are
treated as parts with reasons.
...
The formation simulations support the low-control claim. Across
acquisition, the \textbf{high-threat + low-control} condition produces
the strongest bundle rigidity. \textbf{Chronic low support} produces an
intermediate profile. \textbf{High threat + high control} attenuates
bundle formation rather than eliminating learning altogether: the agent
still learns that threat matters, but helpless self-state consolidates
far less strongly than under low control. That is the important result.
Control does not erase fear learning. It prevents fear learning from
crystallizing into an identity-level bundle.

\section{How Parts Persist}\label{how-parts-persist}

Once formed, parts can persist for decades. The present model explains
that persistence without first assuming literal structural
disconnection.
...
conditions under which they will step back. They often transform after
the exile's burden changes. Those dynamics are clinically central and
...
wholly true when it has the floor. That last feature matters.
Polarization is not simply conflict between two simultaneously
represented preferences. It is often alternation between rival local
realities.
...
It does not yet formalize full protector negotiation. It does not
distinguish genuine Self from self-like managerial imitation. It does
not model the therapist as a second agent, even though the clinical
...
\item
  \textbf{Expository restructuring:} open with an IFS-native account of
  parts, Self, blending, witnessing, and unburdening in clinical
  language, then introduce the computational translation after the
  phenomenology and therapeutic logic are clear.
\item
  \textbf{Visual program redesign:} benchmark the figure set against the
  CBT and coherence-therapy papers, identify the minimum visual grammar
  needed to match that standard, and intersperse explanatory figures
  throughout the paper rather than concentrating them at the end. A
  matrix-style visualization of the model is the minimum starting point.
\item
