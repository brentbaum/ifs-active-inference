#!/usr/bin/env julia
"""
    compare_implementations.jl - Compare custom vs ActiveInference.jl implementations

Runs both implementations on the same spider phobia model and compares:
1. Belief evolution (d-vector updates)
2. Affective learning (a-matrix updates)
3. Behavioral outcomes (approach vs avoid)
"""
...
function run_comparison(; n_trials::Int = 200, spider_dangerous::Bool = false)
    ...
    println("\n[1/2] Running CUSTOM implementation...")
    Random.seed!(42)
    ...
    if ACTIVEINFERENCE_AVAILABLE
        println("\n[2/2] Running ACTIVEINFERENCE.JL implementation...")
        Random.seed!(42)
        ...
        println("\n[2/2] SKIPPING ActiveInference.jl (not installed)")
    ...
    println("=" ^ 70)
    println("\n| Metric                | Custom      | ActiveInference.jl |")
    println("|----------------------|-------------|-------------------|")
    println("| Initial P(safe)      | $(round(custom_p_safe[1]*100, digits=2))%       | $(aif_p_safe !== nothing ? "$(round(aif_p_safe[1]*100, digits=2))%" : "N/A")              |")
    println("| Final P(safe)        | $(round(custom_p_safe[end]*100, digits=2))%       | $(aif_p_safe !== nothing ? "$(round(aif_p_safe[end]*100, digits=2))%" : "N/A")              |")
    println("| Belief shift         | $(round((custom_p_safe[end]-custom_p_safe[1])*100, digits=2))%       | $(aif_p_safe !== nothing ? "$(round((aif_p_safe[end]-aif_p_safe[1])*100, digits=2))%" : "N/A")              |")
    println("| Approach count       | $(custom_results.approach_count)         | $(aif_results !== nothing ? aif_results.approach_count : "N/A")                 |")

    return (custom=custom_results, aif=aif_results, custom_p_safe=custom_p_safe, aif_p_safe=aif_p_safe)
end

# Run comparisons for both conditions
if abspath(PROGRAM_FILE) == @__FILE__
    println("\n" * "=" ^ 70)
    println("SPIDER PHOBIA MODEL: IMPLEMENTATION COMPARISON")
    println("=" ^ 70)
